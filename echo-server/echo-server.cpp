...
void usage() {
	printf("syntax: echo-server [-e[-b]] <port>\n");
	printf("  -e : echo\n");
	printf("  -b : broadcast\n");
...
struct Param {
	...
	bool parse(int argc, char* argv[]) {
		for (int i = 1; i < argc; i++) {
			...
			if (strcmp(argv[i], "-b") == 0 && echo == true) {
				broad = true;
				continue;
			}
			port = atoi(argv[i++]);
...
void recvThread(int sd) {
	...
	while (true) {
		...
		if (param.echo) {
			if(param.broad){
				for(int target : connectList){
					res = ::send(target, buf, res, 0);
					if (res == 0 || res == -1) {
